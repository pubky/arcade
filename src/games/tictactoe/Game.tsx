...
import { LobbyMode, MatchState, TBoard, Square, Actor, TRes } from ".";
import { get } from "./client";

// triggered on click event to make a valid move on the board
// returns false if the square already has a X or O
// returns true if new move is made
const makeMove = (input: { board: TBoard, move: [number,number] }): boolean => {
    const { board, move } = input;

    if (board[move[0]][move[1]] !== Square.EMPTY) {
       return false;
    } else {
        board[move[0]][move[1]] = Square.X;
        // TODO: put updated game state
        return true;
    };
};
...
const checkWin = (board : TBoard) : Square => {
  const lines = [
      [[0, 0], [0, 1], [0, 2]],
      [[1, 0], [1, 1], [1, 2]],
      [[2, 0], [2, 1], [2, 2]],
...
const isValidMove = (localGameState : TRes, otherGameState : TRes) => {
