    ...
    return (<div className={`flex ${align === 'vertical' ? 'flex-col h-fit' : 'w-fit p-0.5'}`}>
        {
            tiles.map((tile, index) => {
                if (index === 0) {
                    return (
                        <ShipTile renderSize={renderSize} isHit={false} key={index} image={align === 'vertical' ? ShipEndUpTile : ShipEndLeftTile} />
                    )
                } else if (index === tiles.length - 1) {
                    return (
                        <ShipTile renderSize={renderSize} isHit={false} key={index} image={align === 'vertical' ? ShipEndDownTile : ShipEndRightTile} />
                    )
                } else {
                    return (
                        <ShipTile renderSize={renderSize} isHit={false} key={index} image={align === 'vertical' ? ShipMidVerticalTile : ShipMidHorizontalTile} />
                    )
                }
            })
        }
